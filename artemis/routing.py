...
"""
The meat of Artemis, doing all the work that actually takes an email and makes
...
The artemis.routing.Router variable (it's not a class, just named like one) is
how the whole system gets to the Router.  It is an instance of RoutingBase and
there's usually only one.

The artemis.routing.StateStorage is what you need to implement if you want
Artemis to store the state in a different way.  By default the
artemis.routing.Router object just uses a default MemoryStorage to do its job.
If you want to use a custom storage, then in your boot modiule you would set
artemis.routing.Router.STATE_STORE to what you want to use.
...
If at any time you need to debug your routing setup just use the artemis routes
command.
...
* @nolocking -- Use this if you want this handler to run parallel without any
  locking around Artemis internals.  SUPER DANGEROUS, add @stateless as well.
...
class RoutingBase(object):
    ...
    glorified module.  It is used mostly internally by the artemis.routing
    ...
    def _collect_matches(self, message):
        ...
        for functions, matchkw in self.match(message.To):
            for func in functions:
                if artemis_setting(func, 'stateless'):
                    yield func, matchkw
    ...
    def deliver(self, message):
        """
        The meat of the whole Artemis operation, this method takes all the
        ...
        It will log which handlers are being run, and you can use the 'artemis route'
        ...
        for func, matchkw in self._collect_matches(message):
            ...
            if artemis_setting(func, 'nolocking'):
    ...
    def call_safely(self, func, message, kwargs):
        ...
        explode and crash.
        """
        from artemis.server import SMTPError
...
class route(object):
    ...
    def setup_accounting(self, func):
        ...
        attach_artemis_settings(func)
        func._artemis_settings['format'] = self.format
        func._artemis_settings['captures'] = self.captures


def artemis_setting(func, key):
    """Simple way to get the artemis setting off the function, or None."""
    return func._artemis_settings.get(key)


def has_artemis_settings(func):
    return "_artemis_settings" in func.__dict__

def assert_artemis_settings(func):
    """Used to make sure that the func has been setup by a routing decorator."""
    assert has_artemis_settings(func), "Function %s has not be setup with a @route first." % func.__name__


def attach_artemis_settings(func):
    """Use this to setup the _artemis_settings if they aren't already there."""
    if '_artemis_settings' not in func.__dict__:
        func._artemis_settings = {}
...
class route_like(route):
    ...
    def __init__(self, func):
        assert_artemis_settings(func)
        self.format = func._artemis_settings['format']
        self.captures = func._artemis_settings['captures']
...
def stateless(func):
    ...
    """
    if has_artemis_settings(func):
        assert not artemis_setting(func, 'format'), "You must use @stateless AFTER @route or @route_like."

    attach_artemis_settings(func)
    func._artemis_settings['stateless'] = True

    return func

def nolocking(func):
    """
    Normally artemis.routing.Router has a lock around each call to all handlers
    ...
    """
    attach_artemis_settings(func)
    func._artemis_settings['nolocking'] = True
    return func
