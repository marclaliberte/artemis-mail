...
confpath = os.path.dirname(os.path.realpath(__file__)) + "/../../../../../artemis.conf"
artemisconf = ConfigParser.ConfigParser()
artemisconf.read(confpath)

HOST = artemisconf.get('database', 'host')
USER = artemisconf.get('database', 'user')
PASS = artemisconf.get('database', 'password')
...
def dbconnect():
    ...
    try:
        ...
        logging.critical("[-] Error (artemisdbconfig.py) - %d: %s" % (e.args[0], e.args[1]))
