"""
A bag of generally useful things when writing unit tests for your Artemis server.
...
The TestConversation will use the artemis.server.Relay you have configured to
talk to your actual running Artemis server.  Since by default Artemis reloads each
...
from artemis import server, routing, mail
from artemis.queue import Queue
...
def spelling(file_name, contents, language="en_US"):
    ...
    in your test settings module.  Use "artemis spell" to make sure it
    works right.
    """
    try:
        ...
        print "Failed to load PyEnchant.  Make sure it's installed and artemis spell works."
        return True
    ...
    if failures:
        print "You have %d spelling errors in %s.  Run artemis spell.." % (failures, file_name)
        return False
    else:
        return True
...
def relay(hostname="127.0.0.1", port=8824):
    """Wires up a default relay on port 8824 (the default artemis log port)."""
...
def clear_queue(queue_dir=TEST_QUEUE):
    """Clears the default test queue out, as created by artemis.testing.queue."""
