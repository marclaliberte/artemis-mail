...
Mako templates.  You MUST configure it by setting artemis.view.LOADER to one of
the template loaders in your boot module.

After that these functions should just work.
"""

from artemis import mail
import email
import warnings
...
def load(template):
    ...
    """
    assert LOADER, "You haven't set artemis.view.LOADER to a loader yet."
...
def respond(variables, Body=None, Html=None, **kwd):
    ...
    on a template.  The only difference from the artemis.mail.MailResponse
...
def attach(msg, variables, template, filename=None, content_type=None,
    ...
    given.  All the parameters that are in artemis.mail.MailResponse.attach
    are there as usual.
    """
