...
from artemis import server, utils, mail, routing, queue, encoding
import artemis
...
Artemis-Version:  %s
Version-File:  %s
""" % (artemis.__version__, artemis.__file__)

copyright_notice = """
Artemis is Copyright (C) Marc Laliberte 2017. Licensed GPLv3.
Merged from Salmon, Copyright (C) Matt Molyneaux 2014-2015. Licensed GPLv3.
If you didn't get a copy of the LICENSE go to:

    https://github.com/marclaliberte/artemis-mail/LICENSE
...
def start_command(parser):
    """
    Runs an Artemis server out of the current directory
    """
...
def stop_command(parser):
    """
    Stops a running Artemis server
    """
    def command(pid, kill=False, all=False):
        ...
        if all:
            ...
            if not os.path.exists(pid):
                print "PID file %s doesn't exist, maybe Artemis isn't running?" % pid
                sys.exit(1)
        ...
        for pid_f in pid_files:
            ...
            print "Attempting to stop Artemis at pid %d" % int(pid)
            ...
            try:
                ...
                print "ERROR stopping Artemis on PID %d: %s" % (int(pid), exc)
...
def status_command(parser):
    """
    Prints out status information about Artemis useful for finding out if it's
    running and where.
    """
    def command(pid):
        if os.path.exists(pid):
            pid = open(pid).readline()
            print "Artemis running with PID %d" % int(pid)
        else:
...
def gen_command(parser):
    ...
    def command(project, force=False):
        template = os.path.join(artemis.__path__[0], "data", "prototype")
...
def cleanse_command(parser):
    """
    Uses Artemis mail cleansing and canonicalization system to take an
