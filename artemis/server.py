...
from artemis import queue, mail, routing, __version__
from artemis.bounce import PRIMARY_STATUS_CODES, SECONDARY_STATUS_CODES, COMBINED_STATUS_CODES
...
# Global dict to store whitelist emial ids
...
class SMTPError(Exception):
    ...
    artemis.bounce to produce them.
    """
...
class Relay(object):
    ...
    important class in the handlers next to the artemis.routing.Router.
    ...
    def send(self, To, From, Subject, Body):
        ...
        then look at artemis.mail.MailResponse.
        """
...
class QueueReceiver(object):
    ...
    records = [] # Global list that will hold spam records unti they're pushed to db.
    ...
    def start(self, scheduler, mailparser, one_shot=False):
        """
        ...
        scheduler.schedule() # Artemis - Start scheduler
        ...
        while True:
            ...
            for key in keys:
                ...
                if msg:
                    ...
                    """
                    Artemis - Interupting normal flow execution of QueueReceiver here and calling our
                    customized module artemismailparser to deal with mails received from the queue.
                    Send "key", which is actually the name of spam file in queue,
                    "msg", is actually complete mail body, in MailRequest format
                    """
                    mailparser.main(key, msg)
