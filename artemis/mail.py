"""
The artemis.mail module contains nothing more than wrappers around the big work
done in artemis.encoding.  These are the actual APIs that you'll interact with
when doing email, and they mostly replicate the artemis.encoding.MailBase 
functionality.
...
import mimetypes
from artemis import encoding, bounce
...
class MailRequest(object):
    """
    This is what older users of Artemis are accustomed to.  The information
    ...
    def is_bounce(self, threshold=0.3):
        ...
        artemis.bounce.BounceAnalzyer given threshold.  0.3 is a good
        conservative base.
        """
        if not self.bounce:
...
class MailResponse(object):
    """
    You are given MailResponse objects from the artemis.view methods, and
