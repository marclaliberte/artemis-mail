...
def notifydeveloper(msg):
    senderid = server.artemisconf.get('notification', 'sender')
    recipient = server.artemisconf.get('notification', 'recipient')
    smtphost = server.artemisconf.get('analyzer', 'relayhost')
    smtpport = server.artemisconf.get('analyzer', 'relayport')
