"""
Mostly utility functions Artemis uses internally that don't
...
from artemis import server, routing
...
def daemonize(pid, chdir, chroot, umask, files_preserve=None, do_open=True):
    ...
    context.stdout = open(os.path.join(chdir, "logs/artemis.out"), "a+")
    context.stderr = open(os.path.join(chdir, "logs/artemis.err"), "a+")
...
def make_fake_settings(host, port):
    ...
    if settings is None:
        ...
        routing.Router.load(['artemis.handlers.log', 'artemis.handlers.queue'])
...
def check_for_pid(pid, force):
    ...
    if os.path.exists(pid):
        if not force:
            print "PID file %s exists, so assuming Artemis is running.  Give -FORCE to force it to start." % pid
            sys.exit(1)
        else:
            os.unlink(pid)
...
def start_server(pid, force, chroot, chdir, uid, gid, umask, settings_loader, debug, daemon_proc):
    ...
    settings.analyzer.start()

    if debug:
        print "Artemis started in debug mode. ctrl-c to quit..."
        import time
        try:
            while True:
                time.sleep(100000)
